...
namespace MyDyb.Handler.Clients.ProjectService.Model
{
    ...
    public partial class CreateFolderRequest : IValidatableObject
    {
        ...
        public CreateFolderRequest(string login = default(string), string name = default(string), long parentId = default(long))
        {
            // to ensure "login" is required (not null)
            if (login == null)
            {
                throw new ArgumentNullException("login is a required property for CreateFolderRequest and cannot be null");
            }
            this.Login = login;
        ...
        IEnumerable<ValidationResult> IValidatableObject.Validate(ValidationContext validationContext)
        {
            ...
            if (this.Login != null && this.Login.Length > 100)
            {
                yield return new ValidationResult("Invalid value for Login, length must be less than 100.", new [] { "Login" });
            }

            // Login (string) minLength
            if (this.Login != null && this.Login.Length < 3)
            {
                yield return new ValidationResult("Invalid value for Login, length must be greater than 3.", new [] { "Login" });
            }
